...
export default function Page(){
  ...
  const crown=()=>{
    ...
    if(wallet<20){ alert("Fund wallet N20"); return; }
    setWallet(v=>v-20);
    setCreatorBal(v=>v+16);
    setAppBal(v=>v+4);
    setCrowns(v=>v+1);
    setEffectText("Crowned N20 - Creator N16 80% App N4 20% - Rose 100");
    setTimeout(()=>setEffectText(""),2000);
  };
  ...
  return(
    <div style={{background:"#050507",color:"#fff",minHeight:"100vh",fontFamily:"sans-serif"}}>
      ...
      <div style={{background:"#00C853",color:"#fff",padding:6,textAlign:"center",fontSize:11,fontWeight:800}}>FIXED BUILD - Followers 5 Following 3 Crowns {crowns} - Rose 100 Creator 80% App 20%</div>
      {showLogo && <div style={{position:"fixed",inset:0,background:"#050507",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",animation:"fadeOut .6s ease 1.9s forwards"}}>
      ...
      {showLogin && <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:50,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
        <div style={{background:"#fff",color:"#000",padding:20,borderRadius:16,width:"100%",maxWidth:320}}>
          ...
          <p style={{fontSize:11,color:"#666",margin:"8px 0"}}>Crown Him N20 Creator 80% App 20% Rose 100 - Verified All From Beginning</p>
      ...
      <main style={{padding:16,maxWidth:480,margin:"0 auto"}}>
        <div style={{background:"#101012",padding:14,borderRadius:12,marginBottom:16}}>
          <p style={{fontSize:13}}>{crowns} Crowns — TikTok shows likes. Instagram shows followers. We show power. 1000 ONLY Lagos. Verified All From Beginning. 842 Active</p>
        ...
        {followers.map(u=><div key={u.id} style={{display:"flex",justifyContent:"space-between",background:"#101012",padding:12,borderRadius:12,marginBottom:8}}><div><p style={{fontSize:13,fontWeight:800}}>{u.name}</p><p style={{fontSize:10,color:"#888"}}>Lv{u.lv} - {u.f} followers</p></div><button onClick={()=>setFollowers(p=>p.map(x=>x.id===u.id?{...x,y:!x.y}:x))} style={{background:u.y?"#CFA658":"#2A2A2E",color:u.y?"#000":"#fff",padding:"6px 14px",borderRadius:18,border:"none",fontSize:11,fontWeight:800}}>{u.y?"Following":"Follow"}</button></div>)}
      </main>
    </div>
  );
}
